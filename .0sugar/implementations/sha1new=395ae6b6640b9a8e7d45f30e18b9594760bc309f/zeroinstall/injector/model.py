...
from zeroinstall.injector import distrokit
...
class DistroKitSource(RetrievalMethod):
	__slots__ = ['id', 'size']
...
class DistroKitImplementation(DistributionImplementation):
	__slots__ = ['os', 'size']

	def __init__(self, feed, id):
		DistributionImplementation.__init__(self, feed, id)
		self.os = None
		self.size = None

	@property
	def installed(self):
		return distrokit.exists(self.id)
...
class ZeroInstallFeed(object):
	...
	def __init__(self, feed_element, local_path = None, distro = None):
		...
		def process_native_impl(item, item_attrs, depends):
			...
			def factory(id):
				impl = self._get_package_impl(id, item_attrs, DistributionImplementation)
				impl.requires = depends
				return impl

			if not distro.get_package_info(package, factory):
				self._packages_to_install.append((item_attrs, depends))
	...
	def pop_postponed(self):
		...
		while self._packages_to_install:
			item_attrs, depends = self._packages_to_install.pop()

			def factory(package_id, version, size):
				id = 'package:%s' % package_id
				impl = self._get_package_impl(id, item_attrs, DistroKitImplementation)

				impl.version = parse_version(version)
				impl.machine = os.uname()[-1]
				impl.size = size

				source = DistroKitSource(id, size)
				impl.download_sources.append(source)

			blocker = distrokit.resolve(item_attrs['package'], factory)
			if blocker is not None:
				postponed.append(blocker)

		return postponed

	def _get_package_impl(self, id, item_attrs, klass):
		assert id.startswith('package:')
		impl = self._get_impl(id, klass)
	...
	def _get_impl(self, id, klass=ZeroInstallImplementation):
		if id not in self.implementations:
			self.implementations[id] = klass(self, id)
